...
module.exports = {
	...
	run: async (client, message, args, prefix, color) => {
		...
		if (client.config.owner.includes(message.author.id)) {
            ...
            embed.setDescription('**Une description complète et détaillée du Bot** <@1310376720196567141>')
            embed.setColor(color)
            .setThumbnail(message.author.avatarURL({ dynamic:true }));
            embed.setTimestamp()
            embed.setFooter(`${client.config.name}`)
            embed.addFields(
                { name: '👑 Owner / Developer :', value: '<@403174893707067392>', inline: true },
                ...
                { name: '📞 Support :', value: `[Rejoindre Le Support](https://discord.gg/6MBqD47Tw6)`, inline: true },
